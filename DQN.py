...
class CarEnv:
    ...
    def step(self, action):
        if action == 0:
            ...
            self.speed = max(self.speed - 2, 6)  # Slow down with minimum speed constraint
        else:
        ...
        for point in self.corners:
            ...
            if 0 <= x < WIDTH and 0 <= y < HEIGHT:
                ...
                if color == BORDER_COLOR:
                    self.alive = False
                    self.collision_penalty = -500  # Penalty for collision with border
                    break
                elif color == STATIONARY_CAR_COLOR:
                    self.alive = False
                    self.collision_penalty = -600  # Penalty for collision with stationary car
                    break
            else:
