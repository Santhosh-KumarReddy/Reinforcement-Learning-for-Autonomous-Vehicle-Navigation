...
class CarEnv:
    def __init__(self):
        ...
        self.map = pygame.image.load('main.map3.png').convert()
...
if __name__ == "__main__":
    ...
    agent.model.load_state_dict(torch.load('ddqn_model.pth'))
    agent.target_model.load_state_dict(torch.load('ddqn_target_model.pth'))
    
    episodes = 700
    episode_rewards = []  # Track rewards
    average_speeds = []  # Track average speeds
    stop_exploring_episode = 200
    ...
    torch.save(agent.model.state_dict(), 'ddqn_model_2_env.pth')
    torch.save(agent.target_model.state_dict(), 'ddqn_target_model_2_env.pth')
